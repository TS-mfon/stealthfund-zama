...
contract StealthCampaign is ZamaEthereumConfig, Pausable, ReentrancyGuard {
    ...
    struct Proposal { bytes32 evidenceHash; uint64 opensAt; uint64 closesAt; bool finalized; bool approved; euint64 forWeight; euint64 againstWeight; }
    ...
    event Refunded(address indexed investor); event ProposalCreated(bytes32 indexed proposalId, bytes32 evidenceHash); event VoteCast(bytes32 indexed proposalId, address indexed voter);
    ...
    function commit(externalEuint64 encryptedAmount, bytes calldata proof) external whenNotPaused nonReentrant {
        ...
        IConfidentialUSD(fundingToken).confidentialTransferFrom(msg.sender, address(this), amount);
        _commitments[msg.sender] = FHE.add(_commitments[msg.sender], amount);
        _totalCommitted = FHE.add(_totalCommitted, amount);
    ...
    function requestFinalization() external {
        ...
        ebool reached = FHE.ge(_totalCommitted, threshold);
        FHE.makePubliclyDecryptable(reached);
        emit FinalizationRequested(ebool.unwrap(reached));
    }

    function finalizeCampaign(bool successful) external {
