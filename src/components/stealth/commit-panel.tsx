...
export function CommitPanel({ campaignAddress = stealthContracts.liveCampaign }: { campaignAddress?: string }) {
  ...
  return <div className="commit upgradedCommit"><div className="commithead"><ShieldCheck/><span>CONFIDENTIAL COMMITMENT</span></div><div className="amountCard"><small>Amount committed</small><label><input placeholder="0.00" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)}/><b>cUSD</b></label><p>Only the encrypted handle/proof is submitted. Observers do not see this amount.</p></div><div className="quickAmounts"><button onClick={() => setAmount("50")} type="button">50</button><button onClick={() => setAmount("100")} type="button">100</button><button onClick={() => setAmount("250")} type="button">250</button><button onClick={() => setAmount("500")} type="button">500</button></div><div className="hiddenrow"><span>Campaign</span><b>{normalizeAddress(campaignAddress).slice(0, 10)}…</b></div><div className="hiddenrow"><span>Status</span><b>{status}</b></div><button disabled={!address || !amount || busy} onClick={commit}>{busy ? "Processing…" : "Encrypt & commit"} <ArrowUpRight size={16}/></button><small>New here? Claim faucet cUSD first, then return to commit privately.</small></div>;
}
