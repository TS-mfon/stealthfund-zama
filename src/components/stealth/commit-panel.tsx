...
export function CommitPanel({ campaignAddress = stealthContracts.liveCampaign }: { campaignAddress?: string }) {
  ...
  async function commit() {
    ...
    try {
      ...
      await publicClient.waitForTransactionReceipt({ hash });
      setStatus("Commit confirmed. Saving portfolio participation globally…");
      const response = await fetch("/api/portfolio", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ chainId: sepolia.id, wallet: address, campaign, tx: hash, createdAt: Date.now() }),
      });
      const body = await response.json().catch(() => null) as { ok?: boolean; error?: string } | null;
      if (!response.ok || !body?.ok) throw new Error(body?.error || "Commit confirmed, but portfolio record could not be saved.");
      setSuccess(`Success: encrypted commitment confirmed. Tx ${hash.slice(0, 10)}…`);
      setStatus("Commit confirmed and added to your global portfolio record. The committed amount remains confidential.");
      setAmount("");
    } catch (error) {
  ...
  return <div className="commit upgradedCommit"><div className="commithead"><ShieldCheck/><span>CONFIDENTIAL COMMITMENT</span></div><div className="balancePill">cUSD balance: <b>confidential</b></div><div className="amountCard"><small>Amount committed</small><label><input placeholder="0.00" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)}/><b>cUSD</b></label><p>Only the encrypted handle/proof is submitted. Observers do not see this amount, and the UI does not fake a plaintext balance.</p></div><div className="quickAmounts"><button onClick={() => setAmount("50")} type="button">50</button><button onClick={() => setAmount("100")} type="button">100</button><button onClick={() => setAmount("250")} type="button">250</button><button onClick={() => setAmount("500")} type="button">500</button></div><div className="hiddenrow"><span>Campaign</span><b>{normalizeAddress(campaignAddress).slice(0, 10)}…</b></div><div className="hiddenrow"><span>Status</span><b>{status}</b></div>{success && <div className="successBox">{success}</div>}<button disabled={!address || !amount || busy} onClick={commit}>{busy ? "Processing…" : "Encrypt & commit"} <ArrowUpRight size={16}/></button><small>New here? Claim faucet cUSD first, then return to commit privately.</small></div>;
}
